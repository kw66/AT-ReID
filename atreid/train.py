...
set_seed(args.seed)
start_time, start = gpu_avaliable(args)
...
criterion_said = saidloss_hdw(cid2pid=dataset.cid2pid, said=args.said, hdw=args.hdw)
...
def train(epoch):
    ...
    for batch_idx, (imgs, pids, cids, mids, camids, indexs) in enumerate(trainloader):
        ...
        if epoch > args.warmup_loss:
            loss2 = criterion_tri(p[0][mids == 1], pids[mids == 1]) + \
                    criterion_tri(p[1][mids == 1], pids[mids == 1]) + \
                    criterion_tri(p[2][mids == 2], pids[mids == 2]) + \
                    criterion_tri(p[3][mids == 2], pids[mids == 2]) + \
                    criterion_tri(p[4], pids) + criterion_tri(p[5], pids)
