...
def attest(args, dataset, model):
    ...
    queryloader1 = DataLoader(
        ImageDataset(dataset.q_adst, transform=transform_test),
        batch_size=args.test_batch, num_workers=args.workers,
        pin_memory=True, drop_last=False, )
    galleryloader1 = DataLoader(
        ImageDataset(dataset.g_adst, transform=transform_test),
        batch_size=args.test_batch, num_workers=args.workers,
        pin_memory=True, drop_last=False, )
    queryloader2 = DataLoader(
        ImageDataset(dataset.q_adlt, transform=transform_test),
        batch_size=args.test_batch, num_workers=args.workers,
        pin_memory=True, drop_last=False, )
    galleryloader2 = DataLoader(
        ImageDataset(dataset.g_adlt, transform=transform_test),
        batch_size=args.test_batch, num_workers=args.workers,
        pin_memory=True, drop_last=False, )
    q, q_pids, q_cids, q_mids, q_camids = extract_feature(model, queryloader1)
    g, g_pids, g_cids, g_mids, g_camids = extract_feature(model, galleryloader1)
...
def attest6(args, dataset, model):
    start = time.time()
    transform_train, transform_test = get_transform(args)
    queryloader1 = DataLoader(
        ImageDataset(dataset.q_adst, transform=transform_test),
    ...
    galleryloader2 = DataLoader(
        ...
        batch_size=args.test_batch, num_workers=args.workers,
        pin_memory=True, drop_last=False, )
    q1, q2, q3, q4, q5, q6, q_pids, q_cids, q_mids, q_camids = extract_feature6(model, queryloader1)
    g1, g2, g3, g4, g5, g6, g_pids, g_cids, g_mids, g_camids = extract_feature6(model, galleryloader1)
    distmat1 = -torch.mm(F.normalize(q1, p=2, dim=1), F.normalize(g1, p=2, dim=1).t()).numpy()

    cmc1, mAP1 = eval(q1, q_pids, q_cids, q_mids, q_camids, g1, g_pids, g_cids, g_mids, g_camids, 'vm', 'sc', distmat1)
    print(f'R-1: {cmc1[0]:.2%} | R-5: {cmc1[4]:.2%} | R-10: {cmc1[9]:.2%} | R-20: {cmc1[19]:.2%} | mAP: {mAP1:.2%}')
    distmat3 = -torch.mm(F.normalize(q3, p=2, dim=1), F.normalize(g3, p=2, dim=1).t()).numpy()
    cmc3, mAP3 = eval(q3, q_pids, q_cids, q_mids, q_camids, g3, g_pids, g_cids, g_mids, g_camids, 'im', 'sc', distmat3)
    print(f'R-1: {cmc3[0]:.2%} | R-5: {cmc3[4]:.2%} | R-10: {cmc3[9]:.2%} | R-20: {cmc3[19]:.2%} | mAP: {mAP3:.2%}')
    distmat5 = -torch.mm(F.normalize(q5, p=2, dim=1), F.normalize(g5, p=2, dim=1).t()).numpy()
    cmc5, mAP5 = eval(q5, q_pids, q_cids, q_mids, q_camids, g5, g_pids, g_cids, g_mids, g_camids, 'cm', 'sc', distmat5)
    print(f'R-1: {cmc5[0]:.2%} | R-5: {cmc5[4]:.2%} | R-10: {cmc5[9]:.2%} | R-20: {cmc5[19]:.2%} | mAP: {mAP5:.2%}')
    ...
    cmc2, mAP2 = eval(q2, q_pids, q_cids, q_mids, q_camids, g2, g_pids, g_cids, g_mids, g_camids, 'vm', 'cc', distmat2)
    print(f'R-1: {cmc2[0]:.2%} | R-5: {cmc2[4]:.2%} | R-10: {cmc2[9]:.2%} | R-20: {cmc2[19]:.2%} | mAP: {mAP2:.2%}')
    distmat4 = -torch.mm(F.normalize(q4, p=2, dim=1), F.normalize(g4, p=2, dim=1).t()).numpy()
    cmc4, mAP4 = eval(q4, q_pids, q_cids, q_mids, q_camids, g4, g_pids, g_cids, g_mids, g_camids, 'im', 'cc', distmat4)
    print(f'R-1: {cmc4[0]:.2%} | R-5: {cmc4[4]:.2%} | R-10: {cmc4[9]:.2%} | R-20: {cmc4[19]:.2%} | mAP: {mAP4:.2%}')
    distmat6 = -torch.mm(F.normalize(q6, p=2, dim=1), F.normalize(g6, p=2, dim=1).t()).numpy()
    cmc6, mAP6 = eval(q6, q_pids, q_cids, q_mids, q_camids, g6, g_pids, g_cids, g_mids, g_camids, 'cm', 'cc', distmat6)
    print(f'R-1: {cmc6[0]:.2%} | R-5: {cmc6[4]:.2%} | R-10: {cmc6[9]:.2%} | R-20: {cmc6[19]:.2%} | mAP: {mAP6:.2%}')
