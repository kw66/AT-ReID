import time
from others.eval import eval, extract_feature
...
def test0(args, d='market', dataset=None, model=None):
    start = time.time()
    t = 1
    ...
    for trial in range(t):
        ...
        if d == 'sysu':
            query = np.array(dataset.query)
            gallery = np.array(dataset.gallery_1[trial])
        if d == 'sysu_indoor':
            query = np.array(dataset.query)
            gallery = np.array(dataset.gallery_i_1[trial])
        if d == 'sysu_multi':
            query = np.array(dataset.query)
            gallery = np.array(dataset.gallery_10[trial])
        if d == 'sysu_indoor_multi':
            query = np.array(dataset.query)
            gallery = np.array(dataset.gallery_i_10[trial])
        if d == 'regdb_v2i':
...
def test(args, dataset=None, model=None):
    if args.d == 'sysu':
        _, _ = test0(args, d='sysu_rgb', dataset=dataset, model=model)
        _, _ = test0(args, d='sysu_ir', dataset=dataset, model=model)
        _, _ = test0(args, d='sysu_multi', dataset=dataset, model=model)
        _, _ = test0(args, d='sysu_indoor', dataset=dataset, model=model)
        _, _ = test0(args, d='sysu_indoor_multi', dataset=dataset, model=model)
        cmc, mAP = test0(args, d='sysu', dataset=dataset, model=model)
    elif args.d == 'llcm':
        cmc1, mAP1 = test0(args, d='llcm_v2i', dataset=dataset, model=model)
        cmc2, mAP2 = test0(args, d='llcm_i2v', dataset=dataset, model=model)
        cmc = cmc1/2+cmc2/2
        mAP = mAP1/2+mAP2/2
    elif args.d == 'regdb':
        cmc1, mAP1 = test0(args, d='regdb_v2i', dataset=dataset, model=model)
        cmc2, mAP2 = test0(args, d='regdb_i2v', dataset=dataset, model=model)
        cmc = cmc1 / 2 + cmc2 / 2
        mAP = mAP1 / 2 + mAP2 / 2
    elif args.d == 'prcc':
        _, _ = test0(args, d='prcc_sc', dataset=dataset, model=model)
        cmc, mAP = test0(args, d='prcc', dataset=dataset, model=model)
    elif args.d == 'ltcc':
        _, _ = test0(args, d='ltcc_all', dataset=dataset, model=model)
        cmc, mAP = test0(args, d='ltcc', dataset=dataset, model=model)
    elif args.d == 'vc':
        _, _ = test0(args, d='vc_sc', dataset=dataset, model=model)
        cmc, mAP = test0(args, d='vc', dataset=dataset, model=model)
    else:
        cmc, mAP = test0(args, d=args.d, dataset=dataset, model=model)
    return cmc, mAP
