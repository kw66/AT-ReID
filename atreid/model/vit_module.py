...
class ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, embed_dim=768,
                 depth=12, num_heads=12, mlp_ratio=4., drop_path_rate=0.):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, dpr=dpr[i])
...
def vit_base_patch16_224_ReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1):
    model = ViT(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12,
        num_heads=12, mlp_ratio=4, drop_path_rate=drop_path_rate)
    return model
